...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        labels_out = torch.zeros((nl, 17)) # I chnaged 6 to 13
        if nl:
...
def verify_image_label(args, blood=True):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            nl = len(lb)
            if nl:
                assert lb.shape[1] == 16, f'labels require 5 columns, {lb.shape[1]} columns detected' # I changed here
                ...
                ne = 1  # label empty
                lb = np.zeros((0, 16), dtype=np.float32)
        else:
            nm = 1  # label missing
            lb = np.zeros((0, 16), dtype=np.float32)
