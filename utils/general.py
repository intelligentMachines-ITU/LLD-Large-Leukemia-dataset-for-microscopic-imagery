...
def non_max_suppression_ps(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Compute conf
        xx= x[:, 5:] ## class prob..
        x[:, 5:] *= x[:, 4:5]  # conf = obj_conf * cls_conf ( multiplay each class with the confidence score)
        ...
        if multi_label:
            ...
            conf, j = x[:, 5:mi].max(1, keepdim=True)
            x = torch.cat((box, conf, j.float(),xx, mask), 1)[conf.view(-1) > conf_thres]
