    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.amp.autocast('cuda'):
                pred,int_feat,deep_feat = model(imgs)  # forward
                # int_feat = [feat.detach() for feat in int_feat_all]
                #batch_obj = 0
                ...
                cell_attribute_loss= cell_training(cell_attribute_model,pooled_feature_map_batch, targets[:,10:17].to(device))
                ...
                loss, loss_items = compute_loss(pred, targets[:,0:10].to(device),deep_feat,epoch)  # loss scaled by batch_size I changed here
                if RANK != -1:
