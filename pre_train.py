...
from utils.loss_o import ComputeLoss
...
def train(hyp, opt, device, callbacks):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(("\n" + "%11s" * 11) % ("Epoch", "GPU_mem", "box_loss_p", "obj_loss_p", "cls_loss_p","box_loss_I", "obj_loss_I", "cls_loss_I","KL_loss", "Instances", "Size"))
        if RANK in {-1, 0}:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                pred,feat,_= model(imgs)  # forward
                loss, loss_items = compute_loss(pred, targets.to(device),feat)  # loss scaled by batch_size
                if RANK != -1:
